...
5. invokeBeanFactoryPostProcessors 执行BFPP 实例化并且执行所有注册的BFPP，在单列对象实例化之前必须要调用
	...
	springboot项目中是通过初始化AnnotationConfigServletWebServerApplicationContext类的时候需要new AnnotatedBeanDefinitionReader类，在new的构造方法中调用父类方法中通过AnnotationConfigUtils进行加载的internalConfigurationAnnotationProcessor
